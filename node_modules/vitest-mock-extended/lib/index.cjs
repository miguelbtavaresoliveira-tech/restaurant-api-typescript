...
const calledWithFn = ({ fallbackMockImplementation, mock: existingMock } = {}) => {
	...
	const fn = existingMock ?? (fallbackMockImplementation ? vitest.vi.fn(fallbackMockImplementation) : vitest.vi.fn());
	...
	fn.calledWith = (...args) => {
		const calledWithFn = fallbackMockImplementation ? vitest.vi.fn(fallbackMockImplementation) : vitest.vi.fn();
...
const mockFn = (existingMock) => {
	if (existingMock !== void 0 && !vitest.vi.isMockFunction(existingMock)) throw new TypeError("mockFn expects a Vitest mock function");
...
exports.isA = isA;
exports.isMockObject = isMockObject;

...
type CalledWithImplementation<Y extends any[], T> = (...args: Y) => T | undefined;
//#endregion
...
type _MockProxy<T> = { [K in keyof T]: T[K] extends ((...args: infer A) => infer B) ? T[K] & CalledWithMock<B, A> : T[K]; };
type MockProxy<T> = _MockProxy<T> & T;
type _DeepMockProxy<T> = { [K in keyof T]: T[K] extends ((...args: infer A) => infer B) ? T[K] & CalledWithMock<B, A> : T[K] & _DeepMockProxy<T[K]>; };
type DeepMockProxy<T> = _DeepMockProxy<T> & T;
type _DeepMockProxyWithFuncPropSupport<T> = { [K in keyof T]: T[K] extends ((...args: infer A) => infer B) ? CalledWithMock<B, A> & DeepMockProxy<T[K]> : DeepMockProxy<T[K]>; };
...
declare const mockFn: <T, A extends any[] = T extends ((...args: infer AReal) => any) ? AReal : any[], R = T extends ((...args: any) => infer RReal) ? RReal : any>(existingMock?: T) => CalledWithMock<R, A> & T;
declare const isMockObject: <T>(obj: T) => obj is MockProxy<T>;
...
declare const calledWithFn: <T, Y extends any[]>({ fallbackMockImplementation, mock: existingMock }?: CalledWithFnArgs<Y, T>) => CalledWithMock<T, Y>;
//#endregion
export { type CalledWithMock, CaptorMatcher, type DeepMockProxy, type GlobalConfig, Matcher, type MatcherCreator, type MatcherFn, type MatcherLike, type MatchersOrLiterals, type MockOpts, type MockProxy, VitestMockExtended, any, anyArray, anyBoolean, anyFunction, anyMap, anyNumber, anyObject, anySet, anyString, anySymbol, arrayIncludes, calledWithFn, captor, isA, isMockObject, mapHas, matches, mock, mockClear, mockDeep, mockFn, mockReset, mocked, mockedFn, notEmpty, notNull, notUndefined, objectContainsKey, objectContainsValue, setHas, stub };
